            ...
            warninp_t wi)
{
  ...
  ITYPE ("polariton",ir->polariton,0);/* 0 is lowest polariton */
  RTYPE ("EMFx",ir->EMFx,0.0);
  RTYPE ("EMFy",ir->EMFy,0.0);
  RTYPE ("EMFz",ir->EMFz,0.0);
  RTYPE ("n_index",ir->n_index,1.0); /* refractive index */
  RTYPE ("L",ir->L,1.0); /* cavity length to be given in micro-meters (then converted to a.u. internally) */
  ITYPE ("n_min",ir->n_min,0); /* integer defining the minimum k=2*Pi*n_min/L_au */
  ITYPE ("n_max",ir->n_max,0); /* integer defining the maximum k=2*Pi*n_max/L_au */
              ...
              warninp_t wi)
{
  ...
  int     nQMmethod,nQMbasis,nQMcharge,nQMmult,nbSH,nbQED,nCASorb,nCASelec,
  ...
  /* cavity QED stuff */
  nbQED      = str_nelem(bQED,MAXPTR,ptr3);
  snew(ir->opts.bQED,nr);
  for(i=0;i<nr;i++){
        ir->opts.bQED[i]      = (gmx_strncasecmp(ptr3[i],"Y",1)==0);
  }
