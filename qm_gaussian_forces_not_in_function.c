...
void init_gaussian(t_commrec *cr, t_QMrec *qm, t_MMrec *mm){
   ...
   */
    ...
    if(qm->bQED){
      ...
      snew(qm->z,ndim-(qm->n_max*2+1));
      zin=fopen("z.dat","r");
      if(zin){
	fprintf(stderr,"z.dat file exists, reading z-positions\n");
	for(i=0;i<ndim-(qm->n_max*2+1);i++){
	...
	fprintf(stderr,"No z-positions file, setting molecules equally spaced along the z-axis\n");
        for(i=0;i<ndim-(qm->n_max*2+1);i++){
	  qm->z[i]=i*qm->L*microM2BOHR/(ndim-(qm->n_max*2+1));
 	  fprintf(stderr,"z[%d]=%lf\n",i,qm->z[i]);
	}
      }
...
real call_gaussian_QED(t_commrec *cr,  t_forcerec *fr, 
  ...
  dplx
    *eigvec,*matrix=NULL,*couplings=NULL,a_sum,betasq,ab,fij,csq;
  double
  ...
  */
  snew(couplings,nmol*(qm->n_max*2+1));
  double E0_norm_sq;
  ...
  for (i=qm->n_max;i>0;i--){
    couplings[m*(qm->n_max*2+1)+qm->n_max-i] = iprod(tdm,u)*sqrt(cavity_dispersion(i,qm)/V0_2EP)*cexp(IMAG*2*M_PI*i/L_au*qm->z[m]);
  }
  for (i=0;i<qm->n_max+1;i++){
    couplings[m*(qm->n_max*2+1)+qm->n_max+i] = iprod(tdm,u)*sqrt(cavity_dispersion(i,qm)/V0_2EP)*cexp(-IMAG*2*M_PI*i/L_au*qm->z[m]);
  }
  /* send couplings around */
  snew(send_couple_real,nmol*(qm->n_max*2+1));
  snew(send_couple_imag,nmol*(qm->n_max*2+1));
  for (i=0;i<nmol*(qm->n_max*2+1);i++){
    send_couple_real[i]=creal(couplings[i]);
    send_couple_imag[i]=cimag(couplings[i]);
  }
  if(MULTISIM(cr)){
    gmx_sumd_sim(nmol*(qm->n_max*2+1),send_couple_real,cr->ms);
    gmx_sumd_sim(nmol*(qm->n_max*2+1),send_couple_imag,cr->ms);
  }
  for (i=0;i<nmol*(qm->n_max*2+1);i++){
  ...
  */
  ...
  if(dodia){
    ...
    for (k=0;k<nmol;k++){
      for (j=0;j<(qm->n_max*2+1);j++){
	matrix[nmol+j+(k*ndim)]= couplings[k*(qm->n_max*2+1)+j];
	matrix[ndim*nmol+k+(j*ndim)]= conj(couplings[k*(qm->n_max*2+1)+j]);
      }
    }
...
//  printM_complex(ndim,eigvec);
  ...
  int 
    n;
  dplx 
    ap_bp,bp_ap,csq2,betasq2,asq_bsq,a_sumq,bp_aq,aq_bp,ap_bq,bq_ap;  

  if(fr->qr->SHmethod != eSHmethodEhrenfest){
    betasq = conj(eigvec[qm->polariton*ndim+m])*eigvec[qm->polariton*ndim+m];

    n=-qm->n_max;
    a_sum = 0.0+IMAG*0.0;
    for (i=0;i<qm->n_max*2+1;i++){
      a_sum += eigvec[qm->polariton*ndim+nmol+i]*sqrt(cavity_dispersion(n++,qm)/V0_2EP);
    }

    bp_ap = conj(eigvec[qm->polariton*ndim+m])*a_sum; //actually sum of alphas * beta_j
    ap_bp = eigvec[qm->polariton*ndim+m]*conj(a_sum);
...
//	fprintf(stderr,"From the computation of Hellman feynman forces, fij=%lf+%lfI\n",creal(fij),cimag(fij));
     ...
     */
    QMener=0.0; 
    int other_m; //other molecules than that corresponding to m
    for (p=0;p<ndim;p++){
      /* do the diagonal terms first p=q. These terms are same as above for 
	...
	if(other_m!=m){
      ...
      a_sum = 0.0+IMAG*0.0;
      for (i=0;i<qm->n_max*2+1;i++){
	a_sum += eigvec[p*ndim+nmol+i]*sqrt(cavity_dispersion(n++,qm)/V0_2EP);
...
//	fprintf(stderr,"From the HF forces, diagonal (p=q) terms, fij=%lf+%lfI\n",creal(fij),cimag(fij));
	...
	a_sumq = 0.0+IMAG*0.0;
	for (i=0;i<qm->n_max*2+1;i++){
	  a_sumq += eigvec[q*ndim+nmol+i]*sqrt(cavity_dispersion(n++,qm)/V0_2EP);
	}
...
//	fprintf(stderr,"From the FH forces, in the off-diagonal (p!=q) terms fij=%lf+%lfI\n",creal(fij),cimag(fij));
  ...
  if((dodia) ){
    ...
    if ((fr->qr->SHmethod != eSHmethodEhrenfest) ){
      ...
      for(ii=0;ii<ndim;ii++){
	fprintf(final_evout,"step %d Eigenvector %d:",step,ii);
	for(k=0;k<ndim;k++){
